 ...
 * iframe at boredfolio.com/agdambagdam — visitors see the product instead of
 * the AuthGate.
 *
 * Runs SYNCHRONOUSLY at module-load (before React paints) so there's no
 * AuthGate flash, and no reload required. A module-level flag prevents the
 * body from running twice under React.StrictMode double-invocation.
 */
let demoAutoLoginApplied = false;
function applyDemoAutoLoginIfNeeded(): void {
  if (demoAutoLoginApplied || typeof window === 'undefined') return;
  demoAutoLoginApplied = true;
  try {
    const params = new URLSearchParams(window.location.search);
    if (params.get('demo') !== '1') return;
    if (!localStorage.getItem('abacus_api_key')) {
