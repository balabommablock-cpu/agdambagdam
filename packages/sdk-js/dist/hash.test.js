...
describe('murmurhash3 — known inputs produce consistent outputs', () => {
    ...
    it('Known reference values are stable across runs', () => {
        ...
        for (const { input, seed, hash } of knownPairs) {
            assert.strictEqual(murmurhash3(input, seed), hash, `murmurhash3('${input}', ${seed}) changed!`);
        }
    });
...
describe('hashToFloat — uniform distribution', () => {
    it('Chi-squared test on 10000 samples across 10 buckets', () => {
        ...
        assert.ok(chiSquared < 30, `Chi-squared = ${chiSquared.toFixed(2)}, expected < 30 for uniform distribution. Buckets: [${buckets.join(', ')}]`);
    });
    it('Deterministic chi-squared test (no randomness) on 10000 samples', () => {
        ...
        assert.ok(chiSquared < 25, `Chi-squared = ${chiSquared.toFixed(2)}, expected < 25 for uniform hash. Buckets: [${buckets.join(', ')}]`);
    });
    it('No bucket has more than 2x expected count', () => {
        ...
        const expectedPerBucket = numSamples / numBuckets;
        for (let i = 0; i < numBuckets; i++) {
            assert.ok(buckets[i] < expectedPerBucket * 2, `Bucket ${i} has ${buckets[i]} samples, expected < ${expectedPerBucket * 2}`);
            assert.ok(buckets[i] > expectedPerBucket * 0.5, `Bucket ${i} has ${buckets[i]} samples, expected > ${expectedPerBucket * 0.5}`);
        }
    });
});
